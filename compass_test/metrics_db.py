...
handful of hops per CLI/frontend run, so connect-on-write is plenty.
"""
...
from . import config
from .models import HopComparison, TestRunReport

SCHEMA_PATH = Path(__file__).resolve().parent / "sql" / "schema.sql"

_INSERT_SQL = """
INSERT INTO rebalancing_hops (
    run_id, journey_index, hop_index, started_at, finished_at,
    run_created_at, live, from_dex, to_dex, journey_stable,
    hop_type, dex, chain, stable, to_stable, to_chain,
    ...
    hop_live, amount_requested_usd, actual_cost_usd, amount_received_usd,
    ...
    %(run_id)s, %(journey_index)s, %(hop_index)s, %(started_at)s, %(finished_at)s,
    %(run_created_at)s, %(live)s, %(from_dex)s, %(to_dex)s, %(journey_stable)s,
    %(hop_type)s, %(dex)s, %(chain)s, %(stable)s, %(to_stable)s, %(to_chain)s,
    ...
    %(hop_live)s, %(amount_requested_usd)s, %(actual_cost_usd)s, %(amount_received_usd)s,
...
ON CONFLICT (run_id, journey_index, hop_index, started_at) DO UPDATE SET
...
def _hop_row(report: TestRunReport, journey_index: int, hop_index: int, hop: HopComparison) -> dict[str, Any]:
...
def upsert_report(report: TestRunReport, conn: psycopg.Connection | None = None) -> int:
    """Flattens report.journeys[*].hops[*] into rebalancing_hops rows and
    upserts them (idempotent on run_id/journey_index/hop_index/started_at —
    safe for reporter.py re-writing latest.json or re-running the backfill
    script). Returns the number of hop rows written."""
    rows = [
        _hop_row(report, ji, hi, hop)
        for ji, journey in enumerate(report.journeys)
        for hi, hop in enumerate(journey.hops)
    ]
    if not rows:
        return 0
    owns_conn = conn is None
...
def summary(days: int = 30, live_only: bool = False) -> dict[str, Any]:
    return _fetch_one(
        ...
        SELECT
            ...
            avg((status = 'ok')::int)::float AS success_rate
        FROM rebalancing_hops
...
def daily_counts(days: int = 30, live_only: bool = False) -> list[dict[str, Any]]:
    return _fetch(
        """
        SELECT time_bucket('1 day', started_at) AS day, count(*) AS hop_count
        FROM rebalancing_hops
        WHERE started_at >= now() - make_interval(days => %(days)s)
          AND (%(live_only)s = false OR live = true)
        GROUP BY day
