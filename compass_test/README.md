...
| Aden | ✅ (untested live) | ✅ (untested live — see caveat) | **BSC only.** Reverse-engineered live 2026-09-07 by capturing the real web app's own network + wallet-signing traffic (see `runners/aden.py` docstring) — withdraw is on a completely separate host pair (`perps.aden.io` / `brokerapi.gateperps.com`) from the HMAC-keyed `api.aden.io` `balances.py::_aden` already uses, with wallet-session JWT auth instead. Withdraw signs whatever `sign_data` hands back verbatim (Orderly Network's public "Withdraw" EIP-712 scheme) rather than a hand-reconstructed struct. **Deposit caveat**: a plain ERC-20 transfer to Aden's own "Deposit" screen address (`COMPASS_TEST_EXPECTED_DEPOSIT_ADDRESS_ADEN_BSC`, MEXC-style), by explicit instruction — this REPLACES an earlier version that called Aden's `Vault.deposit(VaultDepositFE)` contract directly and was the one actually confirmed live end-to-end (real deposit, status progressed 0→10→20). The plain-transfer path has not itself been confirmed to credit the account the same way. **Known gaps, not yet resolved**: the login/token-mint handshake was never captured, so `ADEN_PERP_ACCESS_TOKEN` is a manually-refreshed value that expires in ~15 minutes (needed for withdraw only now — deposit no longer touches the session API at all); two observed request headers (`csrftoken`, `X-GATE-AUTH-TIME`) have unknown generation logic and aren't sent on withdraw calls, so a live withdraw may 403 until that's figured out. |
...
- **Aden's deposit address and `ADEN_PERP_ACCESS_TOKEN` freshness** —
  `COMPASS_TEST_EXPECTED_DEPOSIT_ADDRESS_ADEN_BSC` is a plain transfer
  target off Aden's own UI, not independently confirmed to credit the
  account (see `runners/aden.py`'s docstring — a vault-contract-call path
  that WAS confirmed live exists in this file's git history). Separately,
  `ADEN_PERP_ACCESS_TOKEN` (needed for withdraw) expires in ~15 minutes and
  there's no way yet to mint a fresh one programmatically — copy it out of
  the real site's `localStorage['aden-accessToken']` right before running.
