"""CLI entry point. Run from the compass/ repo root:
    ...
    # journey the hand-set imbalances happen to produce (see
    ...
    # --dex: a swap has no DEX, see runners/cowswap.py). Dry run = a real
    # CoW quote, nothing signed; --live places and waits for a real order:
    ...
    # automatically after each live run:
    ...
    for j in journeys:
        ...
        for h in j.hops:
            ...
            print(
                ...
                f"est.cost=${h.estimatedCostUsd:.4f}  est.time={h.estimatedTimeSeconds:.0f}s ({h.timeSource})  "
                f"solved flow=${h.solvedFlowUsd:.2f}{minLabel}"
            )
    return 0


def _cmd_calibrate_delays(_args: argparse.Namespace) -> int:
    from connectors.dex_measured_delays import DEFAULT_MEASURED_DELAYS_PATH
    from connectors.dex_operational_params import load_dex_operational_params

    measured = calibration.rebuild_measured_delays()
    print(calibration.format_measured_delays_table(measured, load_dex_operational_params()))
    print(f"\nWritten to {DEFAULT_MEASURED_DELAYS_PATH} — measured values now override the configured delays above.")
    return 0
...
def _cmd_run_hop(args: argparse.Namespace) -> int:
    hopType = _HOP_TYPE_BY_ARG[args.hop]
    ...
    cost_err = f"{hc.costErrorPct:+.1f}%" if hc.costErrorPct is not None else "n/a"
    print(f"\n{args.hop} {hc.planned.dex} ({'LIVE' if args.live else 'dry-run'})")
    print(
        f"  est=${hc.planned.estimatedCostUsd:.4f}/{hc.planned.estimatedTimeSeconds:.0f}s  "
        f"actual={'n/a' if hc.executed.actualCostUsd is None else f'${hc.executed.actualCostUsd:.4f}'}"
        f"/{hc.executed.actualTimeSeconds:.0f}s  cost_err={cost_err}  status={hc.executed.status}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    runHop.add_argument(
        ...
        '("Min withdraw" / "Min deposit" in the Config tab), or COMPASS_TEST_SWAP_TEST_USD for a swap',
    )
    ...
    calibrate = sub.add_parser(
        "calibrate-delays",
        help="Rebuild connectors/dex_measured_delays.json from every live run in reports/ and print "
        "configured vs. measured delays (no funds moved, no network)",
    )
    calibrate.set_defaults(func=_cmd_calibrate_delays)
